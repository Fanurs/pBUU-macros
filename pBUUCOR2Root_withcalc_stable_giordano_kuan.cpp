...
Int_t pBUUCOR2Root_withcalc_stable_giordano_kuan(string folder = "")
{
  //=============
  // Definitions
  //=============
  ...
  Double_t mp   = 0.938272013;
  Double_t mn   = 0.939565346;
  ...
  Float_t time = 0;
  Float_t rho = 0;
  //additions above 3/31/2016 and below
  Short_t  px_temp=0,py_temp=0,pz_lab_temp=0,ipid_temp=0, x_temp=0, y_temp=0, z_temp=0, rho_temp=0, t_temp=0;;
  ...
  Double_t part[17] = {0};//total number of read in particles 
  ...
  Float_t param[5]={}; //parameter array
  string pname[5]={"xe","_sn","_energy","_gamma","_b"};
  Int_t pos=0;
  Int_t pos2=0;
  ...
  for(Int_t i = 0; i < 5; i++){
    ...
    param[i] = atof(temp.c_str()); // convert string to Double_t
  }
  ...
  outputfile << pathheader << folder << "_nopions.root";
  stringstream pionoutputfile;
  pionoutputfile << pathheader << folder << "_pion.root";
  ...
  p->cd();
  TTree *pure = new TTree("pure", "COR output");
  ...
  pure->Branch("y",&y);
  pure->Branch("z",&z);
  pure->Branch("time",&time);
  pure->Branch("rho",&rho);
  TTree *derived = new TTree("derived", "COR output");	
  ...
  derived->Branch("time",&time);

  f->cd();
  TTree *pure2 = new TTree("pure", "COR output");
  ...
  pure2->Branch("y",&y);
  pure2->Branch("z",&z);
  pure2->Branch("time",&time);
  pure2->Branch("rho",&rho);
  TTree *derived2 = new TTree("derived", "COR output");	
  ...
  */
  ...
  while(!input.eof()){
    ...
    tempstr = "";
    corfile.str(std::string());
    corfile.clear();
    string filestring; //the x11 file name in the LISN file
    getline(input,tempstr);// reading in cor file
...
//    tempstr=tempstr.substr(0,7);
    ...
    corfile<<pathheader<<"/"<<folder<<"/"<<tempstr;
//    corfile<<pathheader<<"/"<<folder<<"/"<<tempstr<<extension;
    if (tempstr=="") break;
    prev_corfile=tempstr;

    //==================================
    //Open .COR file
    //==================================
    inputfile=fopen(corfile.str().c_str(),"rb");
    cout << corfile.str() << endl;
    ...
    //===============================================
    // begin loop to read in data from .COR files 
    //===============================================
    ...
    while (!feof(inputfile)) {
      ...
      ipid=Int_t(ipid_temp);
      ipid_der = ipid;


      if((ipid>16 || ipid<0) && ipid!=-100 && ipid!=999){ //pid range 0-15 pid=-100 reserved for end of collision pid=999 represents a blank line was read in
      ...
      pz_lab=Float_t(pz_lab_temp);

      fread(&t_temp,sizeof(t_temp),1,inputfile);
      time= Float_t(t_temp);
      fread(&x_temp,sizeof(x_temp),1,inputfile);
      x= Float_t(x_temp);
      fread(&y_temp,sizeof(y_temp),1,inputfile);
      y=Float_t(y_temp);
      fread(&z_temp,sizeof(z_temp),1,inputfile);//deleted _lab_ han
      z=Float_t(z_temp);//changed from z_lab to just z as defined earlier han

      fread(&rho_temp,sizeof(rho_temp),1,inputfile);
      rho=Float_t(rho_temp);
      fread(&header,sizeof(header),1,inputfile);


      //counting the number of events buu outputs pid=px=py=pz-100 at the end of each collision/.COR file
      if (ipid == 999) 
        break;
      if (ipid==-100){ //last line in .COR file 
        ievent = nEmpty; 
        nEmpty++;
        break;
      }
  ...
  ofstream output;
  output.open(pathheader + folder + "/ParticleCount.txt",ios::out|ios::ate|ios::app);  
  Double_t totalparticlesread=part[1]+part[2]+2*part[3]+3*part[4]+3*part[5]+4*part[6]+part[7]+part[8]+part[9];
